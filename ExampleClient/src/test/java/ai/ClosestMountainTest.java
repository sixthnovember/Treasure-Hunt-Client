...
class ClosestMountainTest {

	@Test
	void createGameMap_findClosestMountain_foundExpectedMountain() {
		...
		Point p = positionFinding.getClosestMountain(true);
		Assertions.assertTrue(p == p6);
		p = positionFinding.getClosestMountain(true);
		Assertions.assertTrue(p == p9);
