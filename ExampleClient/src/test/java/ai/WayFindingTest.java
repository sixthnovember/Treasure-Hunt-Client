...
import client.main.WayFinding;
import gamemodel.GameMap;
import map.Field;
import map.Point;
import map.Terrain;

class WayFindingTest {

	@Test
	void createGameMap_getShortestWay_returnsShortestWay() {
		HashMap<Point, Field> gamemap = new HashMap<>();

		Point p1 = new Point(0, 0);
		Field f1 = new Field(false, false, false, Terrain.GRASS, false, false, false);
		gamemap.put(p1, f1);

		Point p2 = new Point(0, 1);
		Field f2 = new Field(false, true, true, Terrain.GRASS, false, false, false);
		gamemap.put(p2, f2);
		...
		Field f9 = new Field(false, false, false, Terrain.MOUNTAIN, false, false, false);
		gamemap.put(p9, f9);
		...
		Point mountainPosition = positionFinding.getClosestMountain(true);
		Point playerPosition = positionFinding.getPlayerPosition();
		WayFinding wayFinding = new WayFinding();
		wayFinding.setGameMap(createdGameMap);
		ArrayList<EMovement> calculatedMoves = wayFinding.getShortestWayTo(playerPosition, mountainPosition);
		ArrayList<EMovement> expectedMoves = new ArrayList<EMovement>(
				List.of(EMovement.RIGHT, EMovement.RIGHT, EMovement.RIGHT, EMovement.RIGHT, EMovement.RIGHT));

		Assertions.assertTrue(calculatedMoves.equals(expectedMoves));
